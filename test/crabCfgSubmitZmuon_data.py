...
if __name__ == '__main__':
    ...
    config.General.workArea     = 'Zmuon_DataJobs_UL2018ABCD_DoubleMu_26Nov2020_removePhase1SoftMuCut'
...
#    config.JobType.sendExternalFolder = True #I don't have an CMSSW_BASE/external so I don't think I need this 
    ...
    config.Data.unitsPerJob     = 10 #Try now with just 10 per job 
...
#     p.join()
    ...
    # Run2018A DoubleMu
    config.General.requestName = 'DoubleMuUL_Run2018A_26Nov2020_removePhase1SoftMuCut' 
    config.Data.inputDataset   = '/DoubleMuon/Run2018A-12Nov2019_UL2018-v2/MINIAOD' 
    config.Data.outLFNDirBase  = '/store/user/mhadley/Zmuon_DataJobs_DiMu_UL2018A_26Nov2020_removePhase1SoftMuCut'
    p = Process(target=submit, args=(config,))
    p.start()
    p.join()
    
  #   Run2018B DoubleMu
    config.General.requestName = 'DoubleMuUL_Run2018B_26Nov2020_removePhase1SoftMuCut' 
    config.Data.inputDataset   = '/DoubleMuon/Run2018B-12Nov2019_UL2018-v2/MINIAOD'
    config.Data.outLFNDirBase  =  '/store/user/mhadley/Zmuon_DataJobs_DiMu_UL2018B_26Nov2020_removePhase1SoftMuCut'
    p = Process(target=submit, args=(config,))
    p.start()
    p.join()
# 
#     Run2018C DoubleMu
    config.General.requestName = 'DoubleMuUL_Run2018C_26Nov2020_removePhase1SoftMuCut' 
    config.Data.inputDataset   = '/DoubleMuon/Run2018C-12Nov2019_UL2018-v2/MINIAOD' 
    config.Data.outLFNDirBase  = '/store/user/mhadley/Zmuon_DataJobs_DiMu_UL2018C_26Nov2020_removePhase1SoftMuCut' 
    p = Process(target=submit, args=(config,))
    p.start()
    p.join()
#     
#     Run2018D DoubleMu    
    config.General.requestName = 'DoubleMuUL_Run2018D_26Nov2020_removePhase1SoftMuCut' 
    config.Data.inputDataset   = '/DoubleMuon/Run2018C-12Nov2019_UL2018-v2/MINIAOD'
    config.Data.outLFNDirBase  =  '/store/user/mhadley/Zmuon_DataJobs_DiMu_UL2018D_26Nov2020_removePhase1SoftMuCut' 
    p = Process(target=submit, args=(config,))
    p.start()
    p.join()
